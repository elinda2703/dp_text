...
The kernel density plot (@fig-kdeplot) demonstrates that deviation from true Moran's I is symmetrical and normally distributed, indicating roughly equal occurrence of positive and negative deviations. Therefore from now on, we can work only with absolute deviation as it doesnt influence.. Interesting observation is, that positive nor negative correlation dont have the tendency to aim towards randomness as edges are being deleted.
...
@fig-shape_size_comapairson also shows that lattice shape, which primarily distinguishes the average number of neighbors per node, does not exert a comparably significant influence on the resulting deviations. Finally, across all lattice sizes, absolute deviation increases approximately linearly with the proportion of missing edges, with no distinct thresholds or stabilization points evident in the data.
...
![Scatterplots of true vs. calculated Moran's $I$ under graph corruption, by percentage of missing edges. Filtered to $n=100$, hexagonal lattice, random edge removal.](figures/p_values4.png){fig-align="center" width="66%"}

![Missclassified statistical significance of Global Moran's $I$. Filtered to random edge removal.](figures/p_values_two_panels_by_error_type.png){#fig-miss_sig fig-align="center" width="66%"}
