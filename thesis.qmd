...
Spatial weights can be represented as a matrix, denoted as $W$, which is a $N \times N$ matrix, where $N$ is the number observations. Each element $w_{ij}$ corresponds to the weight between observations $i$ and $j$, that is, how much observation $j$ influences observation $i$. Because geographic datasets are rarely fully interconnected, most elements in $W$ are 0. To ensure computational and memory efficiency, tools like libpysal [@rey2022PySAL], geoda [@anselin2006GeoDa] or spdep [@spdep] implement sparse matrix, which stores only non-zero elements.
...
Spatial weight matrices are commonly defined trough contiguity, meaning that polygons that share a mutual border are considered to be spatial neigbours. However, this definiton isn't rigorous, leading to several formal interpretations of contiguity. The primary types of contiguity are named analogically to the movements of chess pieces:
...
Rook and Queen contiguity matrices dominate in spatial statistics. They are the only valid choice for regionalization methods and are the default for spatial autocorrelation metrics. Bishop is rarely used on its own, but sometimes it is used for hybrid weights. Beyond immediate neighbours, contiguity can be expanded to higher order contiguity weights (neigbours of neigbours), often used in spatial data modelling.

If we assume a binary contiguity $W$, where $w_{ij}=w_{ji}=1$ if observation $i$ and $j$ are neighbours and $w_{ij}=w_{ji}=0$ if non-neighbours, $W$ is symmetrical and the graph can be represented
...
![US vote map (left) and identified LISA clusters (right).](figures/lisa_and_vote_share_map.png){#fig-lisa_example fig-align="center" width="100%"}
...
tolik a tolik vypoctu bylo performed

### Synthetic data

To get an overview of how size, average number of neigbours and initial spatial autocorrelation influences the deviations of $I$ and its significance under graph corruption, random edge removal was considered. For each lattice type and size, 20 graphs were considered, including the true graph (ground truth) and the most corrupted possible graph for each spatial configuration, those in between represent linear progression of missing edges.

The kernel density plot (@fig-kdeplot) demonstrates that deviation from true Moran's I is symmetrical and normally distributed, indicating roughly equal occurrence of positive and negative deviations. Therefore from now on, we can work only with absolute deviation as it doesnt influence..

![Kernel density plot of deviations from true Moran's I.](figures/moran_deviation_kde_combined.png){#fig-kdeplot fig-align="center" width="60%"}
...
![Edge removal method compaoirson.](figures/method_compairson.png){#fig-global_method_compairson fig-align="center"}

@fig-p-scatter presents a scatterplot of true Moran's I values against their corrupted counterparts across different levels of graph corruption, demonstrated on hexagonal lattice of size 100. It confirms the symmetry of deviations observed in @fig-kdeplot and indicates that datasets exhibiting no significant spatial pattern are more vulnerable to misclassification.

![Scatterplots of true vs. calculated Moran's $I$ under graph corruption, by percentage of missing edges. Filtered to $n=100$, hexagonal lattice, random edge removal.](figures/p_values4.png){#fig-p_scatter fig-align="center" width="66%"}
...
The false non-significance rate increases with corruption level, as illustrated in @fig-miss_sig (right), since more extensive edge removal raises the probability that a given estimate will cross the significance threshold. The magnitude of this rate also reflects dataset size: larger datasets are more stable, and therefore less vulnerable to misclassification. Conversely, average cardinality does not correspond to greater robustness the way it does for $I$ values. Although $I$ deviates more strongly under corruption when cardinality is lower, lattices with lower average cardinality (e.g., square) show lower misclassification rates in @fig-miss_sig (right), while those with higher average cardinality (e.g., hexagonal) show higher rates.

The rate of false significance (@fig-miss_sig, left) is considerably lower than that of false non-significance. It increases up to approximately 15–20% missing edges across all lattice types and dataset sizes, after which it plateaus and fluctuates around a rate of 8%, with no apparent influence from dataset size or cardinality.
...
The critical $I$ values that set the significance threshold can be estimated analytically using the moments derived by Cliff and Ord (1981). Under the randomization assumption, the expectation and variance of $I$ are:

$$E[I] = -\frac{1}{n-1}$$

$$\text{Var}(I) = \frac{n\left[(n^2-3n+3)S_1 - nS_2 + 3S_0^2\right] - b_2\left[(n^2-n)S_1 - 2nS_2 + 6S_0^2\right]}{(n-1)(n-2)(n-3)S_0^2} - E[I]^2$$

where $S_0 = \sum_{i}\sum_{j} w_{ij}$ is the total weight mass, $S_1 = \frac{1}{2}\sum_i\sum_j(w_{ij}+w_{ji})^2$ and $S_2 = \sum_i\left(\sum_j w_{ij} + \sum_j w_{ji}\right)^2$ are weight statistics capturing the structure of $W$, and $b_2$ is the kurtosis of the data. Critically, $S_0$, $S_1$, and $S_2$ depend on the structure of $W$, so the threshold is fully determined by $W$'s connectivity pattern together with one data-dependent scalar, $b_2$. The critical values follow directly by inverting the test statistic:

$$I_{crit} = E[I] \pm z_\alpha\sqrt{\text{Var}(I)}$$

As edges are removed, $S_1$ and $S_2$ shift in a way that increases $\text{Var}(I)$, pushing $I'_{crit}$ further from zero - sparser weight matrices produce a wider, harder-to-clear significance band (Bivand, Müller, and Reder 2009; Cliff and Ord 1981). This means graph corruption has two distinct effects: it perturbs the calculated $I'$ away from its true value, and it independently shifts the critical threshold itself.

Significance loss occurs when \$\|I\| \> \|I\_{crit}\|\$ and \$\|I'\| \< \|I'\_{crit}\|\$. As corruption increases, \$\|I'\_{crit}\|\$ grows, causing a larger share of computed values \$I'\$ to fall within the widened non-significant interval. Notably, even where \$I'\$ moves away from zero, \$I'\_{crit}\$ may still exceed it, since the threshold shifts independently of the realized value of \$I'\$.

This mechanism operates in the opposite direction for significance gain, which occurs when \$\|I\| \< \|I\_{crit}\|\$ and \$\|I'\| \> \|I'\_{crit}\|\$. If \$I'\$ moves toward zero under corruption, the observation necessarily remains non-significant. If \$I'\$ moves away from zero, it may in principle exceed the threshold; however, this outcome is comparatively rare, as \$I'\_{crit}\$ is simultaneously shifting in the same direction. This asymmetry explains why significance gain occurs substantially less frequently than significance loss.

The occurence of significance loss and gain is illustrated in @fig-sig_hist.

![Significance misclassification rate per size. Data filtered to random edge removal.](figures/moran_falsesig_falsenonsig_rate_by_size_3panel.png){#fig-sig_hist fig-align="center"}

### Real data

The deviations of $I$ for real datasets

![Moran's I deviation on real data under graph corruption (upper left, upper right, lower left) and $p$-value change of Guerry dataset (lower right). Each run is plotted separately. Data filtered to random edge removal.](figures/sac_real_overview.png){#fig-sac_real fig-align="center" width="66%"}

-local a boders rmoval nezaslouzi plot, protoze max deviation je tam tak 0.003. zaslouzi si max tak odstavec
...
![](figures/reg_real_examples.png)

![Method of edge removal compairson on regionalization techniques. Binned by 1% of missing edges.](figures/reg_method_compairson_binned1.png){#fig-reg_nevim fig-align="center" width="66%"}
...
![Example](figures/reg_ceara_zika_map.png){#fig-reg_zika}

![](figures/reg_chicago_map.png)

![](figures/reg_south_map.png)
