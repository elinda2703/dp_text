...
Spatial objects that share a common border are considered to be contiguous. However, "sharing a common border" isn't a rigorous definition, leading to several formal interpretations of contiguity. The primary types of contiguity are named analogically to the movements of chess pieces:
...
Furthermore, these base definitions can be expanded into higher-order contiguity weights (neighbours of neigbours) to model relationships beyond immediate neighbours.
...
-   Single linkage: dissimilarity is measured based on the minimum distance between any pair of observations in the two clusters.
-   Complete linkage: dissimilarity is measured based on the maximum distance between any pair of observations in the two clusters.
-   Average linkage: dissimilarity is measured based on the maximum distance between any pair of observations in the two clusters.
-   Ward's linkage: see below.
...
All observations are considered to be in one region and they are connected by the initial MST, which has exactly $n-1$ edges. The MST is pruned by removing an edge which increases the between-cluster dissimilarity the most. This is accessed trough calculating the sum of squared deviations ($SSD_T$) calculated as
...
Developed by @guo2008, Regionalization with dynamically constrained agglomerative clustering and partitioning (REDCAP) consists of both merging and divisive processes. It combines SCHC and SKATER.

The first part of the algorithm is ideologically identical to SCHC, it builds an entire dendrogram. It has a dissimilarity matrix, which we can also think of as a complete graph. That is a graph, that connects all pairs of possible observations with assigned weight based on their dissimilarity. The dissmilarity is based on the linkage parameter (see chap x). In the original paper, @guo2008 omits Ward's linkage, however, it is implemented in Geoda [@anselin2006GeoDa].

REDCAP has an additional parameter on what observations should be checked for dissimilarity between clusters. @guo2008 distinguishes two constraining strategies when computing the dissimilarity between two clusters:

-   First-order: only edges connecting two neigbouring individual observations (not clusters), that sit on the border for the two clusters (regions) and are in different clusters at the same time, are considered.
-   Full-order: All edges connecting observations from different clusters are considered. This is also used in SCHC.

During each iteration, the pair of neigbouring clusters that has the smallest dissimilarity is merged, until all bservation belong to one cluster and the whole dendrogram is built. More importantly, during this agglomerative process, MST is being constructed. During each interation when the two clusters are selected for merging, an edge is added to the MST. This edge always corresponds to the first-order single-linkage...

After the whole MST is built, the pruning process is the same as in SKATER, meaning $k-1$ edges are removed from the MST to reach $k$ regions. When using REDCAP's First-Order Single Linkage, the MST is identical the the SKATER's one (and the final result as well), as it indirectly uses Kruskal's algorithm.

#### AZP

Automatic Zoning Procedure (AZP) was proposed by @openshaw1977 and extended in @openshaw1995. Unlike the approaches discussed above, AZP is not hiearchical.

At the beginning, usually randomly, it delineats $p$ contiguus regions. For each region the within cluster $SSD$ is calculated following the equation ... Total within $SSD$ corresponds to the sum of all within cluster $SSD$s. The algorithm then randomly picks one of those regions (called $c$) and it identifies its neighbouring observations from different regions (called $C$). From $C$, it randomly selects an observation $i$ and directly removes it from the list $c$. $i$ is now a candidate to be removed from its original region and become a part of region $C$. For that to happen, it must satisfy two rules:

-   it must not break the contiguity of the original region,
-   it must decrease the total within $SSD$.

If the two rules are satisfied, $i$ is added to region $C$ and $c$ is updated for new neigbouring observations gained by adding $i$. This process is repeated until $C$ is empty. After $C$ is empty, the algorithm selects a different region and repeats the whole process, until it goes trough all the regions.

AZP doesn't provide an analytical solution. It doesn't guarantee a global optimum and it can easily get trapped in a local optimum @anselin2024 . AZP therefore has multiple other parameters that enhace the results of the algorithm.

One of those parametrs is search option, which... It includes tabu search, which prevents observations to switch back to its original region during the next few iterations. Another one is Simulated Annealing, using the metropolois algorithm to...

The other parameter is regarding the intial regionalization. The *automatic regionalization with initial seed location* (ARiSeL), seeks to... Other option is to draw the initial region delineation from other regionalzation methods, such as SCHC or SKATER.

#### Max p-regions problem

Proposed by @duque2012, max p-regions problem seeks to find maximum number $p$ of contigous regions, so that each of these regions meets a minimum size constraint, ususally being a sum of one attribute value across the observations (e. g. population).

The first phase in the algorithm is the growth. An observation $i$ is randomly selected. It is checked if it its attribute value (lets say population) is larger than a treshold. If it is, $i$ is its own cluster (singleton). If its not, its neigbours are identified and the one with biggest population joins $i$ in the cluster. Combined population of those two is calculated. If its bigger tnah the treshold, the region is complete for now. If it isnt, the neigbours are updated for the new observation and the process is repeated unitl the treshold is reached. After the treshold is reached, another unassigned observation is selected and the process is repeated.

With the growth phase, isolated unassigned observations or multiple contigous observations that together dont meet the population constrint occur. Those are called enclaves. Each of them has to be assigned to an exsiting neigbouring region frm the growth phase. If the enclave is neigbouring more than one region, the decision is made to minimize the overall $SSD$. If there are multiple such enclves, all posibble combinations of joining are computed and checked for the overall $SSD$.

The outcome after the growth and enclave assignment is the initial feasible solution. Now its time for optimizing process like with AZP...
...
As contiguity detections directly rely on topological properties of a polygonal dataset, contiguity can be defined trough DE9-IM matrix as well. In libpysal, contiguity is detected trough checking the matrix, where Queen must satisfy the touches spatial predicate. Rook is defined trough as ...
...
Geoplanar [@geopanda2026], a python package developed for planar enforcement of Geopandas datasets, among overlaps and gaps, also considers non-planar touches and edges as a violation. In their presence, contiguity detection is compromised as two polygons are considered a Rook contiguity neighbours if they share at least two consecutive vertices. These vertices need to be present and encoded in both polygons, which differs from a pure spatial predicate testing overlapping boundaries, as this might not be encoded as two consecutive vertices in one of the neighbours if the shared part is just a slice of an edge. Similarly with Queen contiguity - two polygons are considered a neighbour if they share a vertex. While some tools (e.g. libpysal) allow both of these definitions, a default is the strict vertex coincidence - a potential point of topology corruption in real-world data.
...
-(mis)specification of weight matrix

-clanky o uncertainty of attributues, missing data a their implications + propagation of error?
