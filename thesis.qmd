...
@fisher1999models distinguishes three primary forms of uncertainty in spatial data: error, vagueness, and ambiguity. Error occurs when a geographical object is well-defined, but its recorded measurement or position deviates from the objective truth. Vagueness arises when the boundaries of an object are not clearly defined or crisp. Finally, ambiguity happens when there are multiple differing definitions or classifications for a single object or attribute, an issue fundamentally tied to subjective human interpretation.
