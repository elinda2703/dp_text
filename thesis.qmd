...
<!-- where two polygons are considered a neighbour if they share at least two consecutive vertices. These vertices need to be present and encoded in both polygons, which differs from a pure spatial predicate testing overlapping edges, as this might not be encoded as two consecutive vertices in one of the neighbours if the shared part is just a slice of an edge. While some tools (e.g. libpysal) allow both of these definitions, a default is the strict vertex coincidence - a potential point of topology corruption in real-world data. -->
...
To simulate erroneous topology, controlled adjacency corruptions are applied to a set of topologically correct polygons. These corruptions consist of removing edges from the $W$ reflecting Rook contiguity of polygons. Missing adjacencies are spatially distributed in three ways:

-   Randomly, where adjacency relations are removed uniformly at random across the entire graph. Any edge can be deleted.
-   On region borders, simulating errors arising from the integration of multiple spatial datasets. Any two of the predefined regions can be deleted.
-   Locally, where adjacencies are removed within a spatially contiguous subregion, representing localised human digitisation or editing errors. One of predefined regions is selected and edges within it as well as those who only have one node in the region, can be deleted.

![Deletable edges: random (left), region borders (center), local (right)](figures/cor_method.png){#fig-cor_method fig-align="center" width="80%"}

Since spatial autocorrelation measures aren't explicitely defined for observations with no neighbours, reffered to as isolates, their occurence had to be prevented while corrupting $W$. What this means is that if at least one of two observations connected by an edge has a node degree of 1 (has only 1 neigbour), the edge cannot be deleted. Those have to be subtracted from the deletable edges subgraph.

In similar fashion, regionalization techniques are not defined for if $W$ has more than one component. Therefore edges known as bridges, after whose deletion the graph would split into more components need to be kept. therefore subtracted from deletable edges subgraph. In case the edges are removed randomly, it will end up in MST.